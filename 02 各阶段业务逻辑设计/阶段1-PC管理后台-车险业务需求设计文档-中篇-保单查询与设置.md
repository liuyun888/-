...
2. 点击【新增同步配置】，弹窗填写：
   - 目标商户：输入框搜索（按商户名称/编号搜索）。
   - 同步工号：选择作为同步凭证的保司工号（该工号须在 A、B 两个商户下均已配置）。
   - 是否启用：开关。
3. 保存配置，写入 `ins_car_policy_sync_config` 表。

**同步触发机制：**
- A 商户新录入保单后，检查是否存在该工号的同步配置（`company_no_id` 匹配且启用）。
- 若存在，发送 MQ 消息 `ins_car_policy_sync`，消费者在 B 商户下创建同样的保单记录（`merchant_id` 替换为 B 商户，`creator` 标注为「同步」）。

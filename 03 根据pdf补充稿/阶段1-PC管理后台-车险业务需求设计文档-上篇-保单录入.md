...
排查步骤（前端逐步引导展示）：
1. 使用录单时所选的工号，登录对应保险公司系统（出单中心），在保司系统内查询目标保单。
2. 如果保司系统内查询不到该保单：核实出单工号是否与实际出单工号相符；联系保险公司核实保单状态。
3. 如果保司系统内可以查到但仍无法录单：联系系统客服协助处理（可能是保单详情字段解析异常）。

**类型二：工号错误**

系统提示：`工号错误`

含义：录单时选择的工号，其在保险公司系统中的密码错误，或配置的机构代码/出单网址有误，导致无法登录保司系统。

排查步骤：
1. 确认工号密码是否正确（是否已被修改）。
2. 检查工号配置中的机构代码和出单网址是否与保司要求一致。
3. 联系所属代理公司总部管理员核实并修改录单工号密码。

**类型三：未成功建立连接**

系统提示：`未成功建立连接`

含义：系统无法连接到保险公司系统，通常为网络原因（如人保需要 VPN 连接，但当前环境无 VPN 或未切换出单网）。

排查步骤：
1. 手动点击录单页面的「切换出单网」按钮，切换后重新录单尝试。
2. 如仍然提示此错误，联系系统客服协助处理。

#### 排查步骤展示逻辑（分步引导 UI）

系统展示对应错误类型的排查步骤，用户按步骤操作：

**Step 1：工号状态检查**
- 系统自动调用工号健康检测接口，检查当前所选工号在保司系统的登录状态。
- 若工号异常（未配置 / 已过期 / 被锁定 / 密码错误），显示具体原因：`工号[xxx]状态异常：[原因]，请联系管理员重新配置`。
...
| 批改原因 | 文本 | ✅ | 最大 200 字 |
| 批改日期 | 日期 | ✅ | 不能早于原保单签单日期 |
| 批单号 | 文本 | ✅ | 批改保单的新单号，建议格式：原保单号-001 |
| 批改保费变动 | 金额 | 条件必填 | 增保/减保类型时必填；正数=增保（如加保200元填「200」），负数=减保（如退保500元填「-500」）；支持小数 |
| 批改后保费 | 金额（展示） | — | 系统自动计算：原保费 + 批改保费变动 |
| 支付日期 | 日期 | ✅ | 无论加保还是退保，支付日期和批单日期均需改为最新批单日期（用于对账） |

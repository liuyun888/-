...
>
> **变更说明（V2.1）：** 对照排期表（阶段3-PC管理后台-营销管理）补充功能点PDF编号标注，并补充了以下缺失细节：
> - 活动创建：补充积分兑换类型活动的规则配置；
> - 活动列表：补充撤回审核操作细节；
> - 优惠券创建：补充兑换码批量生成机制详细说明；
> - 优惠券发放：补充自动发放事件类型完整列表；
>
> **PDF编号对照说明：** 活动管理与优惠券管理为平台自研营销功能，操作手册中无独立PDF文档。

---

## 一、活动管理

> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `活动管理`
> - `活动创建`（前端4天 + 后端3天）
> - `活动审核`（前端1.5天 + 后端1.5天）
> - `活动列表`（前端2天 + 后端1.5天）
> - `活动数据统计`（前端2.5天 + 后端2天）
>
> 📌 **技术难点**：多类型活动规则配置动态渲染；产品批量Excel导入；复杂规则JSON存储；状态机定时任务；MQ批量通知参与用户；ECharts多类型图表。
...
**积分兑换活动：** *(排期表新增，活动类型第7种)*
- 兑换品来源：选择「积分商城」中已上架的兑换品，支持多选；
- 活动限时折扣：可设置活动期间所需积分减免百分比（0-50%），0表示不打折；
- 每人限兑次数：正整数，0表示不限；
- 活动期间库存：可单独设置活动专属库存（独立于积分商城库存），-1表示不限；
- 触发条件：可选「直接参与」（活动页面展示）或「购买后解锁」（购买指定产品后获得兑换资格）；
...
**撤回审核：**
- 待审核状态（status=1）的活动，创建者可点击「撤回」按钮；
- 弹出确认框，确认后：status: 1 → 0（草稿），audit_status重置为0；
- 已发送的审核通知不撤回，但审核员在列表中该活动会消失（不再待审）；
- 撤回后可继续修改活动信息，再重新提交；
...
> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `优惠券`
> - `优惠券创建`（前端3天 + 后端2天）
> - `优惠券发放`（前端2天 + 后端2.5天）
> - `优惠券列表`（前端1天 + 后端0.5天）
> - `核销统计`（前端1.5天 + 后端1天）
>
> 📌 **技术难点**：兑换码批量生成（MQ异步）；Redis分布式锁防超发；乐观锁更新库存；定时任务自动解锁锁定券；退款还券逻辑。

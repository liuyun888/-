...
> 文档版本：V2.1 | 编写日期：2026-02-26 | 定位：开发实现指南，聚焦业务逻辑与操作细节
>
> **变更说明（V2.1）：** 对照排期表（阶段3-PC管理后台-营销管理）及操作手册PDF，补充了各功能点的PDF对应编号标注，并完善了部分缺失的实现细节。
>
> **PDF编号对照说明：** 本模块（内容管理）为平台自研功能，在操作手册中暂无直接对应的独立PDF文档，但与以下模块存在联动：
> - Banner/文章/知识库/视频均为C端商城（消费者前台）的内容来源，与阶段3-C端商城内容展示接口直接关联；
> - 视频管理与阿里云VOD配置参见平台技术规范文档；
> - 培训管理课程与视频管理共用部分OSS上传能力（参见PDF-225课程管理）。

---

## 一、Banner 管理

> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `内容管理` → `Banner管理`
> 📌 **操作手册**：本功能为平台自研，操作手册中无独立PDF编号，但C端展示逻辑与阶段3-C端商城首页Banner展示直接关联。
...
> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `内容管理` → `文章管理`
> 📌 **操作手册**：本功能为平台自研内容管理功能，无独立操作手册PDF。文章发布后在C端商城"资讯"频道展示。
...
> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `内容管理` → `知识库管理`
> 📌 **操作手册**：本功能为平台自研，无独立操作手册PDF。知识库内容在C端商城"保险学堂"或"帮助中心"频道展示，支持全文检索。
...
> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `内容管理` → `视频管理`
> 📌 **操作手册**：本功能为平台自研，无独立操作手册PDF。视频课程上传能力与培训管理模块（PDF-225课程管理）共用，但入口独立。阿里云VOD相关配置参见平台基础设施规范。

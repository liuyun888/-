...
> **文档版本**：V2.0（基于操作手册 PDF 97-106 补充完善）
...
> 来自 PDF 98（险别占比）、PDF 100（保险公司分析）的指标缩写说明：

所有统计分析中，各指标含义统一如下：

| 指标名（列头缩写） | 计算公式 | PDF中缩写说明 |
|--------|----------|------|
| 单量 | COUNT(policy_id) | 保单数量 |
| 总（总保费） | SUM(total_premium) | 含税保费合计 |
| 净（净保费） | SUM(net_premium) | 净保费合计 |
| 收（上游手续费） | SUM(upstream_fee) | 向保险公司收取的手续费 |
| 支（下游手续费） | SUM(downstream_fee) | 付给业务员/机构的手续费 |
| 利（利润） | 上游手续费 - 下游手续费 | 即收-支（来自 PDF 98：利=上游手续费-下游手续费） |
| 率（利润率） | 利润 / 净保费 × 100% | 即利/净（来自 PDF 100：率=利/净） |

---

## 二、非车险别占比分析

> **对应 PDF：`98_非车统计分析非车险别占比分析`**
> **对应排期**：非车险别占比分析，前端1天 + 后端1天
...
> 来自 PDF 98：点击【非车】—【统计分析】—【险别占比分析】，可通过筛选条件，筛选出要查看的信息；根据上面的筛选条件，筛选出来的各个保险公司的数据分析图，可以根据需要，修改要查看的数据信息以及显示分析的图表类型；最下面显示的是各个保险公司的单量、总保费、净保费、上游手续费、下游手续费的数据统计。
...
- 图表右上角提供「切换图表类型」按钮，可在饼图/柱状图间切换（来自 PDF 98：可以根据需要，修改要查看的数据信息以及显示分析的图表类型）。

**底部：数据明细表**

按险种分组统计，表格字段（来自 PDF 98 指标说明）：

| 险种名称 | 单量 | 总（总保费） | 净（净保费） | 收（上游手续费） | 支（下游手续费） | 利（利润） |
|---------|------|------------|------------|----------------|----------------|-----------|
| 财产险 | ... | ... | ... | ... | ... | ... |
| 工程险 | ... | ... | ... | ... | ... | ... |
| 责任险 | ... | ... | ... | ... | ... | ... |
...
> **对应 PDF：`99_非车统计分析非车分支机构分析`**
> **对应排期**：非车分支机构分析，前端1.5天 + 后端1.5天
...
> 来自 PDF 99：点击【非车】—【统计分析】—【分支机构分析】，可通过筛选条件，筛选出要查看的信息；根据上面的筛选条件，筛选出来的各个保险公司的数据分析图，可以根据需要，修改要查看的数据信息以及显示分析的数据类型。
...
- X 轴：机构名称；Y 轴：净保费。
- 可通过图表右侧下拉切换显示「总保费」「净保费」「单量」等维度（来自 PDF 99：可以根据需要，修改要查看的数据信息以及显示分析的数据类型）。
...
> **对应 PDF：`100_非车统计分析非车保险公司分析`**
> **对应排期**：非车保险公司分析，前端1天 + 后端1天
...
> 来自 PDF 100：点击【非车】—【统计分析】—【保险公司分析】，可通过筛选条件，筛选出要查看的信息；根据上面的筛选条件，可查看各个保险公司的单量占比和保费占比；最下面显示的是各个保险公司的数据统计，包括各个保险公司的单量，总保费，净保费，收支利率等信息。
...
按保险公司分组统计，表格字段（列头缩写含义来自 PDF 100：收=上游手续费，支=下游手续费，利=收-支，率=利/净）：

| 保险公司 | 单量 | 总（总保费） | 净（净保费） | 收（上游手续费） | 支（下游手续费） | 利（利润） | 率（利润率） |
|---------|------|------------|------------|----------------|----------------|-----------|------------|
| 平安财险 | ... | ... | ... | ... | ... | ... | ... |
| 人保财险 | ... | ... | ... | ... | ... | ... | ... |
| ... | | | | | | | |
...
> **对应 PDF：`101_非车统计分析非车区域占比分析`**
> **对应排期**：非车区域占比分析，前端1天 + 后端1天
...
> 来自 PDF 101：通过上面的筛选条件，筛选出的各个区域的数据分析图；展示各个区域对应的组织机构的数据分析，含括单量，总保费，净保费，上下游手续费以及利润。
...
**底部：数据明细表**
- 按省市+机构+保险公司三维度交叉展示数据（来自 PDF 101：展示各个区域对应的组织机构的数据分析）。
...
> **对应 PDF：`102_非车统计分析非车业务来源分析`**
> **对应排期**：非车业务来源分析，前端1天 + 后端1天
...
> 来自 PDF 102：通过上述筛选条件，筛选出的各个业务来源的单量、总保费、净保费、上下游手续费，利润。

**顶部**：筛选条件区

**中部：图表区**
- 各渠道（直销/转介绍/经纪/代理/其他等）的**业务量占比饼图**和**保费占比柱状图**（来自 PDF 102：筛选出的各个业务来源的数据分析图）。
...
> **对应 PDF：`104_非车系统设置产品管理`**
> **对应排期**：产品管理（系统产品+自定义产品），前端1天 + 后端1天

### 7.1 入口

导航：【非车】→【系统设置】→【产品管理】

页面包含**四个 Tab**（来自 PDF 104 补充）：**险种类别** | **系统产品** | **系统产品方案** | **自定义产品**

---

### 7.2 险种类别 Tab

> 来自 PDF 104：点击【非车】-【产品管理】-【险种类别】查看。
...
> 来自 PDF 104：点击【非车】-【产品管理】-【系统产品】查看。注：**系统产品仅适用于标的标识是车辆的保单**。
...
### 7.4 系统产品方案 Tab（补充自 PDF 104）

> 来自 PDF 104：点击【非车】-【产品管理】-【系统产品方案】查看。

展示系统产品对应的产品方案列表（只读）：
- 查询条件：保险公司、产品名称
- 列表字段：保险公司、产品名称、产品方案名称、状态
- 用于政策配置中「产品方案」字段的数据源
...
> 来自 PDF 104：系统中没有的产品支持自定义添加。注：**此处自定义的产品，在保单录入时无论标的标识是人、物品、车辆，均可勾选**。
...
> **重要提示（来自 PDF 104）**：非车保单表格导入中产品和险种类别对应下图信息。**一定要保持表格中的产品与类别，与系统中的一致才能导入成功**。
...
> **对应 PDF：`105_非车系统设置模板设置`**
> **对应排期**：模板设置（导入模板配置），前端1.5天 + 后端1.5天
...
> 来自 PDF 105：点击[非车]—-[系统设置]—[模板设置] 点击【新增模板】可设置新的导出模板。
...
> 来自 PDF 105：项目大类不能更换顺序，同一项目下的小类条件可以拖动更换顺序。

字段分为「未选字段区」和「已选字段区」：
- 左侧「未选字段」：按大类分组，**大类顺序不可调整**，组内字段可单个勾选：
  ...
  - **批单信息**：原保单号、批改类型等
- 右侧「已选字段」：已勾选的字段列表，**同一大类内的小类可拖拽调整顺序，大类顺序不变**（来自 PDF 105）。
...
来自 PDF 105：已经新增过的模板在列表可进行查看、编辑、禁用。
...
CREATE TABLE `ins_non_vehicle_template_field` (
  ...
  `sort`            INT          DEFAULT 0 COMMENT '大类内排序',
  `group_sort`      INT          DEFAULT 0 COMMENT '大类排序（固定，不可调整）',
  PRIMARY KEY (`id`),
...
> **对应 PDF：`106_非车系统设置保单设置`**
> **对应排期**：保单设置（查询字段与行为配置），前端0.5天 + 后端0.5天

### 9.1 入口

导航：【非车】→【系统设置】→【保单设置】

> **重要（来自 PDF 106）**：修改的内容都需要点击保存，并且**重新登录才可以生效**。

---

### 9.2 录单设置（一）：保单业务归属设置

> 来自 PDF 106：非车保单业务归属默认是业务员，点击选项，可以改为出单员，或者录单员。**修改后非车保单统计中将会按照您所选择的归属进行汇总筛选**。
...
> 来自 PDF 106：可以针对组织进行勾选，选中并保存后，该组织的人员在修改保单的**下游手续费及跟单手续费**时，填写的费用**不能超过保单已匹配的费用**。
...
> 来自 PDF 106：勾选对应险种并保存后，录单时就可以录入该险种下相同保单号不同产品的保单了。
...
> 来自 PDF 106：这两项功能是我们技术针对后期非车理赔模块做的铺垫，**目前尚未投入使用，敬请期待后续功能的发布**。
...
| `/non-vehicle/product/system-plan-list` | GET | 系统产品方案列表 |
...
| `/non-vehicle/settings/save` | POST | 保存保单设置（保存后需重新登录生效） |
...
│   └── /list               # 非车政策列表
├── /statistics
│   ├── /insurance-type     # 险别占比分析（PDF 98）
│   ├── /org                # 分支机构分析（PDF 99）
│   ├── /company            # 保险公司分析（PDF 100）
│   ├── /region             # 区域占比分析（PDF 101）
│   └── /business-source    # 业务来源分析（PDF 102）
└── /settings
    ├── /product            # 产品管理（含险种类别/系统产品/系统产品方案/自定义产品四Tab）
    ├── /template           # 模板设置
    └── /policy-config      # 保单设置（修改后需重新登录）
```
...
| 功能点 | 难点说明 |
|--------|----------|
| 险别占比分析 | ECharts饼图；险种编码分类映射；动态分组聚合；图表类型切换 |
| 分支机构分析 | 递归机构树统计；按保司动态分列；与车险分支机构分析共用底层统计组件 |
| 保险公司分析 | ECharts柱状图；动态险种分列；保司维度聚合 |
| 区域占比分析 | ECharts地图热力图；区域字段从被保标的地址获取 |
| 业务来源分析 | 业务来源字段从保单录入记录；渠道分类可配置 |
| 产品管理 | 系统产品与自定义产品分开管理；产品名称全局唯一校验；与保单导入、政策配置联动；四Tab设计 |
| 模板设置 | 模板字段配置化存储；字段分组与保单表字段元数据对应；大类顺序固定，小类支持拖拽；导入时按模板解析列映射 |
| 保单设置 | 配置项存sys_config表；修改后需重新登录生效；影响统计分析归属维度 |

---

*文档版本：V2.0 | 对应排期表：阶段2-PC管理后台-非车险业务 | 参考操作手册：`97`、`98`、`99`、`100`、`101`、`102`、`103`、`104`、`105`、`106`号*

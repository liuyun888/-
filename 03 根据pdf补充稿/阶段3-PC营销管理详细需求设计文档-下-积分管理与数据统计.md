...
>
> **变更说明（V2.1）：**
> 1. 对照排期表补充各功能点的排期工时和PDF对应编号；
> 2. 补充积分规则配置中积分等级体系详细说明；
> 3. **新增第三章：培训管理模块**（对应操作手册PDF-222至PDF-228），本章为V2.1新增内容。

---

## 一、积分管理

> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `积分管理`
> - `积分规则配置`（前端2天 + 后端2天）
> - `积分发放`（前端1天 + 后端2天）
> - `积分兑换（积分商城）`（前端3天 + 后端2天）
> - `积分明细`（前端1.5天 + 后端1天）
>
> 📌 **技术难点**：规则引擎灵活配置；MQ事件驱动；积分等级倍率计算；数据库行锁防并发；先进先出过期处理；Redis分布式锁防并发。
...
> 📋 **功能对应排期表**：`阶段3-PC管理后台-营销管理` → `数据统计`
> - `实时看板`（前端3天 + 后端2天）
> - `访问统计`（前端2天 + 后端2天）
> - `转化漏斗`（前端2.5天 + 后端2天）
> - `销售统计`（前端2.5天 + 后端2天）
>
> 📌 **技术难点**：Redis实时计数；HyperLogLog统计UV；WebSocket/轮询5分钟刷新；ECharts多图表；多步骤数据聚合；汇总表与明细表双读策略。

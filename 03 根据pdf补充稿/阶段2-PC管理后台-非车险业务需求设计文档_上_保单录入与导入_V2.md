...
> **文档版本**：V2.0（基于操作手册 PDF 86、87、88、93 补充完善）
...
> **对应 PDF：`86_非车保单管理非车保单录入`**
> **对应排期**：非车保单录入（手工单笔），前端2天 + 后端2天
...
| 标的标识 * | 单选：车辆 / 人 / 物品 | 是 | **核心分支字段**，控制后续表单展示结构，详见下方分支逻辑 |
| 标的名称 | 文本 | 否 | 被保标的物名称 |
| 保(批)单号 * | 文本 | 是 | 与保险公司组成联合唯一索引，防止重录 |
| 险种编码 | 文本/下拉 | 否 | 与险种联动 |
| 险种 * | 下拉选择 | 是 | 加载非车险种字典 |
| 产品名称 * | 下拉选择 / 文本输入 | 是 | **分支逻辑**：标的标识=**车辆**时，只能从系统产品中选择；标的标识=**人或物品**时，产品名称为**自定义文本输入**，不限于系统产品 |
| 互联网业务 | 单选：是/否 | 否 | — |
| 涉农业务 | 单选：是/否 | 否 | — |
...
| 共保标识 * | 单选：是/否 | 是 | 选"是"时出现共保保司填写区域；**注：共保单录入仅在标的标识为人或物品时支持** |

#### 共保保司信息（共保标识=是时显示，仅标的标识=人或物品时支持）

> 来自 PDF 86：当标的标识选择人或物品时，支持共保单录入，共保标识选择【是】，添加多家共保保司进行保存。
...
> 来自 PDF 86：标的标识可选车辆、人或物品，根据选择需要填写的信息不同。

| 标的标识 | 显示内容 |
|----------|----------|
| **车辆** | 车主姓名、证件类型、证件号、车牌号、车架号、发动机号；车主/被保人可勾选"同投保人"自动填充；**注：选车辆时，关系人信息必填** |
| **人** | 被保人姓名（支持添加多个被保人），每人：姓名、证件号、出生日期；关系人信息中被保人可以添加多人信息 |
| **物品** | 被保人姓名、证件号；关系人信息只有投保人和被保人，填写信息保存即可 |

> **注意**：选车辆时，车主和被保人为**必填**关系人信息。选人/物品时，被保人可为空或多人。
...
CREATE TABLE `ins_non_vehicle_policy` (
  ...
  `input_type`            VARCHAR(50)  COMMENT '录入方式：手工录单/自动录单等',
  `is_co_insured`         TINYINT(1)   DEFAULT 0 COMMENT '共保标识：0-否 1-是（仅人/物品标的支持）',
  ...
  `total_premium`         DECIMAL(18,4) NOT NULL COMMENT '全保费（含税）',
  `net_premium`           DECIMAL(18,4) NOT NULL COMMENT '净保费（不含税）',
  ...
  `upstream_settle_status` TINYINT     DEFAULT 0 COMMENT '上游结算状态：0-未结算 1-已结算',
  `downstream_rate`       DECIMAL(8,4)  COMMENT '下游手续费比例(%)',
  `downstream_fee`        DECIMAL(18,4) COMMENT '下游手续费金额',
  `downstream_settle_status` TINYINT   DEFAULT 0 COMMENT '下游结算状态：0-未结算 1-已结算',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_company_policy_no` (`tenant_id`, `insurance_company_id`, `policy_no`, `product_id`) COMMENT '联合唯一索引（开启相同保单号不同产品时含product_id）'
...
CREATE TABLE `ins_non_vehicle_co_insurer` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_policy_id` (`policy_id`)
) ENGINE=InnoDB COMMENT='非车险共保信息子表（仅适用于标的标识为人或物品的保单）';
```
...
> **对应 PDF：`87_非车保单管理非车批单录入`**
> **对应排期**：非车批单录入，前端1天 + 后端1.5天
...
1. **输入原保单号**：在「原保单号」输入框粘贴/输入原非车保单的保(批)单号，点击【查询】或失焦时自动触发：后端根据 `tenant_id + 原保单号` 查询 `ins_non_vehicle_policy`，回填以下信息（只读展示）：
   - 保险公司名称
   - 险种
   - 关联产品名称（来自 PDF 87：页面会直接带出此非车保单关联非车产品）
   - 原保单净保费
   - 原保单上游手续费比例
...
| 批改保单号 * | 文本 | 是 | 如：原保单号-001（来自 PDF 87 示例） |
| 批改日期 * | 日期 | 是 | 批改生效日期 |
| 支付日期 * | 日期 | 是 | **无论加保退保，支付日期和批单日期都要改为最新批单日期**（来自 PDF 87：为了对账使用） |
| 批改保费差额 * | 金额（可负数） | 是 | **加保填正数**（如200直接写200）；**退保填负数**（如退保500填-500）；单位：元（来自 PDF 87） |
...
> **对应 PDF：`88_非车保单管理非车保单导入`**
> **对应排期**：非车保单导入（Excel批量导入），前端1.5天 + 后端2天

### 3.1 入口

导航：【非车】→【保单管理】→【保单查询】→点击【保单批量导入】按钮（来自 PDF 88：点击【非车】-【保单管理】-【保单查询】-【保单批量导入】）。
...
- Excel 文件包含两个 Sheet：
  - **sheet1**：数据录入区，第一行为字段表头（**红色标注为必填字段**，来自 PDF 88），从第二行开始录入数据。
  - **sheet2**：导入字段说明/示例（来自 PDF 88：sheet2 可以参照导入名称示例），每个字段的填写规范和枚举值参考。

#### Step 2：填写数据
- 用户按照 sheet1 表头填写非车保单数据。
- **关键注意**：「险种类别」和「产品名称」字段的值必须与系统中【非车-系统设置-产品管理】内的数据**完全一致**（包括全角/半角、空格）。
- 日期格式要求：`yyyy-MM-dd`（如 `2024-01-15`），不能用 `/` 分隔。

#### Step 3：上传文件
- 回到系统，点击【请选择要导入的数据文件】或拖拽上传 Excel 文件（来自 PDF 88）。
...
#### Step 6：查看导入任务状态
- 导入提交后，用户可点击【任务列表】查看当前及历史导入任务（来自 PDF 88：导入完成后，可点击任务列表，查看对应状态）：
...
> **对应 PDF：`93_非车保单管理非车不能导入的排查步骤`**
> **对应排期**：非车不能导入的排查步骤，前端0.5天 + 后端0.5天

此为页面引导功能，无独立路由，在导入失败时在错误弹窗底部展示「常见原因排查」折叠面板。

### 4.1 系统自动提示的错误类型与排查步骤

> 来自 PDF 93 实际案例：校验提示"产品与系统内的产品匹配不上"时，需在【非车】【产品管理】【自定义产品和系统产品】都查询；若查不到需新增后重新导入。

| 错误代码 | 错误提示 | 排查步骤（来自 PDF 93） |
|----------|----------|------------|
| `ERR_PRODUCT_NOT_FOUND` | 产品与系统内的产品匹配不上 | ①查看表格中「产品名称」值；②到【非车】→【产品管理】，分别在「系统产品」和「自定义产品」Tab搜索；③若查不到，点击【新增产品】添加后重新导入 |
| `ERR_TYPE_MISMATCH` | 险种和产品归属不匹配 | ①查看表格中「险种类别」与「产品名称」；②到产品管理找到该产品，确认系统中该产品对应的险种类别；③**以系统中的险种类别为准**，更新表格后重新导入（来自 PDF 93：在自定义产品查询到险种类别为企业财产保险，而表格填的是伤害险，根据系统信息更改表格数据再导入） |
...
| 功能点 | 难点说明 |
|--------|----------|
| 非车保单录入 | 非车字段多且险种差异大；字段按险种动态显隐；联合唯一索引(保司+保单号)防重录；标的标识驱动产品选择分支逻辑 |
| 非车批单录入 | 批单与原保单关联；保费差额导致手续费重算；批改历史版本归档；支付日期必须更新为批单日期 |
| 非车保单导入 | EasyExcel异步导入；险种/产品名称精确匹配校验；批次管理；导入失败详情报告 |
| 导入失败排查 | 导入失败错误码与提示语映射表；支持下载含错误说明的失败明细Excel |

---

*文档版本：V2.0 | 对应排期表：阶段2-PC管理后台-非车险业务 | 参考操作手册：`86`、`87`、`88`、`93`号*

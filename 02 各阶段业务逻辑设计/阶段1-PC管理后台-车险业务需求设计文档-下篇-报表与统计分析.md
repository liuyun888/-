...
3. 数据来源说明：手续费=上游结算金额，各细分项暂由人工录入或从财务模块同步（阶段6实现，当前阶段显示「暂无数据」）。
...
与监管报表同数据源，展示格式以财务部门内部使用为准（列可以增加：净保费、佣金率、环比等）。
...
| 保险公司 | 件数 | 总保费 | 净保费 | 收（上游手续费） | 支（下游佣金） | 利（收-支） | 费率（利/净保费）|
|----------|------|--------|--------|-----------------|---------------|-----------|-----------------|

> 收/支/利/费率 字段在佣金系统（阶段2）完善后才有数据，本阶段展示 0 或「--」。
